  ...
  login(
    ...
    async (loginError, api) => {
      ...
      ;(() => {
          ...
          for (const file of items) {
            try {
              ...
              if (!config || !run || (type === 'commands' && !config.commandCategory)) {
                throw new Error(
                  `Lỗi định dạng trong ${type === 'commands' ? 'lệnh' : 'sự kiện'}: ${file}`
                )
              }
              if (global.client[collection].has(config.name)) {
                throw new Error(
                  `Tên ${type === 'commands' ? 'lệnh' : 'sự kiện'} đã tồn tại: ${config.name}`
                )
              }
              if (config.envConfig) {
                ...
                for (const key in config.envConfig) {
                  global.configModule[config.name][key] =
              ...
              console.error(
                `Lỗi khi tải ${type === 'commands' ? 'lệnh' : 'sự kiện'} ${file}:`,
                error
              )
            }
          }
        ...
        const loadedEventsCount = loadModules(
          eventPath,
          'events',
        ...
        if (error) {
          if (JSON.stringify(error).includes('601051028565049')) {
            ...
            api.httpPost('https://www.facebook.com/api/graphql/', form, (e, i) => {
              ...
              if (e || res.errors)
                return logger('Lỗi không thể xóa cảnh cáo của facebook.', 'error')
