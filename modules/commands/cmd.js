...
const loadCommand = ({ moduleList, threadID, messageID }) => {
  ...
  if (errorList.length !== 0) {
    api.sendMessage(
      `Những module đã xảy ra sự cố khi đang load: ${errorList.join(' ')}`,
      threadID,
      messageID
    )
  }
  ...
  switch (command) {
    case 'load':
      if (moduleList.length === 0)
        return api.sendMessage('Tên module không được để trống!', threadID, messageID)
    ...
    case 'loadall': {
      const loadAllModules = readdirSync(__dirname).filter(
    ...
    case 'unloadall': {
      const unloadAllModules = readdirSync(__dirname).filter(
        (file) => file.endsWith('.js') && !file.includes('example') && !file.includes('command')
      )
    ...
    case 'info': {
      ...
      if (!commandInfo)
        return api.sendMessage('Module bạn nhập không tồn tại!', threadID, messageID)
