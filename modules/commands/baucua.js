  ...
  try {
    ...
    for (const choice of choices) {
      ...
      if (winCount > 0) {
        ...
        winDetails.push(`❌ ${choice.charAt(0).toUpperCase() + choice.slice(1)} → -${formatMoney(betPerChoice)}`);
      }
    }
    ...
    try {
      ...
      await api.sendMessage({ body: dataTrave, attachment: fs.createReadStream(mergedPath) }, threadID, messageID);
      await fs.remove(mergedImagePath);
    } catch (error) {
      await api.sendMessage(`${dataTrave}\n🚫 Lỗi hiển thị hình ảnh: ${error.message}`, threadID, messageID);
    }
  } catch (error) {
