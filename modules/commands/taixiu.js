...
async function parseBet(bet, userMoney) {
  ...
  if (normalizedBet.endsWith('%')) {
    ...
    if (Number.isNaN(percent) || percent < 1 || percent > 100) return [null, '🚫 Phần trăm phải từ 1-100'];
...
module.exports.run = async ({ api, event, args, Currencies, Users }) => {
  ...
  if (!['tài', 'xỉu'].includes(choice)) return api.sendMessage("🚫 Chọn 'tài' hoặc 'xỉu'", threadID, messageID);

  // Chạy song song để giảm lag
  const [userData, userCurrency] = await Promise.all([Users.getData(senderID), Currencies.getData(senderID)]);
