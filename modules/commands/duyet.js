...
module.exports.handleReply = async ({ event, api, handleReply }) => {
  ...
  const pendingThreads = JSON.parse(fs.readFileSync(dataPendingPath))
  ...
  if (handleReply.type === 'pending') {
    if (body.trim().toLowerCase() === 'all') {
      ...
      return api.sendMessage(
        `✅ Phê duyệt thành công toàn bộ ${pendingThreads.length} nhóm`,
        threadID,
        messageID
      )
    }
    ...
    for (const num of numbers) {
      ...
      if (index >= 0 && index < pendingThreads.length) {
        ...
        api.sendMessage(
          '✅ Nhóm của bạn đã được phê duyệt!\n📝 Chúc các bạn dùng bot vui vẻ',
          idBox
        )
    ...
    return successCount > 0
      ...
      : api.sendMessage(
          '❎ Không có nhóm nào được phê duyệt, vui lòng kiểm tra lại số thứ tự',
          threadID,
          messageID
        )
  } else if (handleReply.type === 'remove') {
    const idsToRemove = body
      .split(' ')
      .map((num) => parseInt(num, 10) - 1)
      .filter((index) => approvedThreads[index])
    if (idsToRemove.length) {
      ...
      fs.writeFileSync(dataPath, JSON.stringify(approvedThreads, null, 2))
      return api.sendMessage(
        `✅ Đã xóa các box:\n${idsToRemove.map((index) => approvedThreads[index]).join(', ')}`,
        threadID,
        messageID
      )
    }
...
module.exports.run = async ({ event, api, args, Threads }) => {
  ...
  if (args[0] === 'list' || args[0] === 'l') {
    ...
    for (const [index, id] of approvedThreads.entries()) {
      const name = (await Threads.getData(id)).threadInfo.name || 'Tên không tồn tại'
      msg += `\n${index + 1}. ${name}\n🧬 ID: ${id}`
    }
    return api.sendMessage(
      `${msg}\n\n📌 Reply theo stt để xóa nhóm`,
      threadID,
      (error, info) => {
        if (!error) {
  ...
  if (args[0] === 'pending' || args[0] === 'p') {
    ...
    for (const [index, id] of pendingThreads.entries()) {
      const threadInfo = (await Threads.getData(id)).threadInfo
      msg += `\n${index + 1}. ${threadInfo.threadName}\n🧬 ID: ${id}`
    }
    return api.sendMessage(
      `${msg}\n\n📌 Reply theo stt để duyệt nhóm`,
      threadID,
      (error, info) => {
        if (!error) {
          global.client.handleReply.push({
  ...
  if (args[0] === 'help' || args[0] === 'h') {
    ...
    return api.sendMessage(
      `[ Duyệt Box ]\n\n` +
        ...
        `${prefix}${this.config.name} => kèm theo ID để duyệt box đó`,
      threadID,
      messageID
    )
  }
  ...
  if (Number.isNaN(parseInt(idBox, 10))) {
